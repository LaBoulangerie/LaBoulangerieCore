...
public class EventCmd implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        switch (args[1].toLowerCase()) {
            ...
            case "nextstep":
                ...
                break;
            case "status":
                sender.sendMessage("§bThe event §e" + event.getName() + " §b is currently "
                        + (event.hasStarted() ? "§arunning §7(" + event.getStage() + "/" + event.getTotalStages() + ")"
                                : event.hasEnded() ? "§4ended" : "§7off"));
                for (int i = 0; i < event.getSteps().size(); i++) {
                    sender.sendMessage(
                            (i <= event.getStage() - 1 ? "§a├ " : "§7├ ") + event.getSteps().get(i).getName());
                }
                break;
            case "goto":
                ...
                if (event.goTo(args[2].replaceAll("_", " "))) {
                    ...
                    sender.sendMessage("§cCouldn't jump to step: " + args[2].replaceAll("_", " ")
                            + ", either it doesn't exist or the event isn't running.");
                }
                break;
            case "run":
                if (args.length < 3) return false;
                if (event.run((Player) sender, args[2].replaceAll("_", " "))) {
                    ...
                    sender.sendMessage("§cCouldn't run step: " + args[2].replaceAll("_", " ")
                            + ", either it doesn't exist or the event isn't running.");
                }
                break;
            default:
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if (args.length == 2)
            suggestions = Arrays.asList("start", "stop", "nextStep", "reset", "goto", "run", "status");
        ...
        if (args.length == 3 && (args[1].equalsIgnoreCase("goto") || args[1].equalsIgnoreCase("run"))) {
            ...
            if (state != null) suggestions = state.getSteps().stream().map(step -> step.getName().replaceAll(" ", "_"))
                    .collect(Collectors.toList());
        }
        return suggestions == null ? new ArrayList<>()
                : suggestions.stream().filter(str -> str.startsWith(args[args.length == 0 ? 0 : args.length - 1]))
                        .collect(Collectors.toList());
    }
}
